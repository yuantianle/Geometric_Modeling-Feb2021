...
public:
  ...
  glm::vec3 E2;
  glm::vec3 N;
  Eigen::SparseMatrix<float> axesSelf;
  float relativeX;
  float relativeY;
  float relativeZ;
  glm::vec3 lplsPos;
  std::map<int, Eigen::Matrix3f> mapMatrixOtherToSelf;
  std::map<int, float> mapWeightForOther;
  float fTotalWeight;
...
//------------------------------------
  ...
  //----------Deformation---------------
  void GetFrameForV();
  void RIDeforming(Polyhedron* poly);
  void UpdateDiscreteFrame(Vertex* vertex);

  //----------------------------------------------------------------------------------------
  void InitAxesAndMatrixSelf();
  void InitNewAxesMatrix();
  double m_iAroundNumber;   //NEED TO SET NUMBERS
  double m_iHeightNumber;   //NEED TO SET NUMBERS
  Eigen::SparseMatrix<float> m_NewAxesMatrix;
  void CalculateNewAxes();
  void InitLpls();
  Eigen::SparseMatrix<float> m_Matrix;
  void CalculateNewMatrixLpls();

  std::vector<int> m_fixedindex;
  std::vector<int> m_editedintex;

  float m_rotate_theta;
  //------------------------------------
